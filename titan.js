...
const privateKey = data.pvtkey
const tokenAddress = data.cntrctadrss
const fromAddress = data.fromAddrss
const contractABI = [
  // transfer
  {
      'constant': false,
...
db.collection('pendUsers').insertOne({userId: ctx.from.id})
 ...
 await  ctx.replyWithMarkdown('*🔍 Please Complete This Captcha To Prove Yourself Human*\n\n▶️ *Please Answer* : `'+q1+'` + `'+q2+'` = ?\n\n☑ _Send you Answer Below!!!!_',  { reply_markup: { keyboard: [['♻️ Try Again']], resize_keyboard: true } })
...
ctx.replyWithMarkdown(
    '*✔️ Dashboard*',
    { reply_markup: { keyboard: [['💳 Balance'],['👫 Referral', '🎰 Bonus', 'Withdraw 💵'], ['ℹ Token Information', '⚙️ Set Wallet']], resize_keyboard: true }})
...
sup = sum*1
    ...
    ctx.reply('👤 *User:- '+ctx.from.first_name+'*\n\n🧰 *Balance:- '+sum.toFixed(3)+' '+bot_cur+'*',{parse_mode:'markdown'})
} catch (err) {
    sendError(err, ctx)
  }
})

bot.hears('⚙️ Set Wallet', async (ctx) => {
try {
if(ctx.message.chat.type != 'private'){
  return
  }
  let dbData = await db.collection('allUsers').find({userId: ctx.from.id}).toArray()
    ...
    ctx.replyWithMarkdown('💡 *Your '+data.chain+' Wallet is:* `'+ dbData[0].coinmail +'`',
   Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change '+data.chain+' Wallet', 'iamsetemail')]
      ])
      )  
       .catch((err) => sendError(err, ctx))
    }else{
ctx.replyWithMarkdown('💡 *Your '+data.chain+' Wallet is:* _not set_', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change '+data.chain+' Wallet', 'iamsetemail')]
      ])
      ) 
...
bot.action('iamsetemail', async (ctx) => {
  ...
  ctx.deleteMessage();
    ctx.replyWithMarkdown(
      '✏️ *Send your '+data.chain+'  Wallet Address From MetaMask OR TrustWallet\n\n(©️ Contract Address =* `'+data.cntrctadrss+'`)',{ reply_markup: { keyboard: [['🔙 back']], resize_keyboard: true }})
        .catch((err) => sendError(err, ctx))
...
if(check.length===0){
ctx.replyWithMarkdown(
'🖊* Done:* Your '+data.chain+' Wallet Has Been Saved as\n`'+ctx.message.text+'`',
...
ctx.reply('Seems This '+data.chain+' Wallet have been used in bot before by another user! Try Again')
}
}else{
 ctx.reply('🖊 Error: This is not a valid '+data.chain+' Wallet Send /start to return to the menu, or send a correct one')
 }
} catch (err) {
    sendError(err, ctx)
  }
...
bot.hears('ℹ Token Information',async (ctx) => {
  
  ctx.replyWithMarkdown('*🔍 Network : '+data.chain+' Blockchain\n©️ Contract :* `'+data.cntrctadrss+'`\n*🔮 Name : '+bot_cur+'\n🔮 Decimal : 18*')  })

bot.hears('⏺ Continue', async (ctx) => {
	ctx.replyWithMarkdown('🕊 Follow *Airdrop NFT* on [Twitter](https://twitter.com/'+data.tweet+'), Like and Retweet pinned post.\n\n❇ Click "*☑ Done*" After Following!!!!!',{ disable_web_page_preview: true, reply_markup: { keyboard: [['☑ Done']], resize_keyboard: true } })
	})
...
let dbData = await db.collection('allUsers').find({userId: ctx.from.id}).toArray()
    ...
    if ('coinmail' in dbData[0]) {
if(bal>=min_wd){
var post="*🩸 Send the amount you want to withdraw*\n\n    *Minimum:* `"+min_wd.toFixed(3)+"` "+bot_cur+"\n    *Maximum:* `"+bal.toFixed(3)+"` "+bot_cur+"\n    _Maximum amount corresponds to your balance_\n\n    ☑️* Send now the amount of you want to withdraw*"

ctx.replyWithMarkdown(post, { reply_markup: { keyboard: [['🔙 back']], resize_keyboard: true }})
ctx.scene.enter('onWithdraw')
}else{
ctx.replyWithMarkdown("❌ *You have to own at least "+min_wd.toFixed(3)+" "+bot_cur+" in your balance to withdraw!*")
}
    }else{
    ctx.replyWithMarkdown('💡 *Your '+data.chain+' Wallet Address:* `not set`', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change Your '+data.chain+' Wallet', 'iamsetemail')]
      ])
      ) 
           .catch((err) => sendError(err, ctx))
...
db.collection('vUsers').updateOne({stat: 'stat'}, {$set: {value: sta}}, {upsert: true})
       ...
       contract = new Web3js.eth.Contract(contractABI, tokenAddress, { from: fromAddress });
       const amount = Web3js.utils.toHex(Web3js.utils.toWei(`${msg}`));
       let data = contract.methods.transfer(wallet, amount).encodeABI();
       ...
       if (err) {
           ...
           return Web3js.eth.sendSignedTransaction(signedTx.rawTransaction, (err, res) => {
               if (err) {
                   ...
                   ctx.replyWithMarkdown('_😢 Unfortunately Your Withdrawal Request Has Been Failed By The '+data.chain+' Servers.\n\n✖️ Dont Worry !! You failed Withdraw Issue Has Been Successfully Reported To Bot Administrator.\n\n☑️ Admin Team Will Pay you Within Next 48 Hours.\n\n🔵 Please Be Patient❗️_')
    bot.telegram.sendMessage(admin,'✖️ <b>'+bot_cur+' Withdraw Failed!</b>\n------------------------------------------------\n👤 <b>User : </b><a href="tg://user?id='+ctx.from.id+'">'+ctx.from.first_name+'</a>\n💵 <b>Amount : '+msg+'</b>\n📦 <b>Wallet : </b> <code>'+wallet+'</code>',{parse_mode: 'HTML', disable_webpage_preview: 'true' })
               } else {
                  ctx.replyWithHTML('<code>$'+bot_cur+' Wɪᴛʜᴅʀᴀᴡᴀʟ Pʀᴏᴄᴄᴇᴅᴇᴅ!!!!</code>\n<code>User --> </code><b><a href="tg://user?id='+ctx.from.id+'">'+ctx.from.first_name+'</a></b>\n<code>Amount --> '+msg+' '+bot_cur+'</code>\n<code>Wallet --> </code><code>'+wallet+'</code>\n<code>Payment Status --> </code><b><a href="https://polygonscan.com/tx/'+res+'">Successfully Paid✓</a></b>')
                  bot.telegram.sendMessage('@PayoutWorld_1','<code>$'+bot_cur+' Wɪᴛʜᴅʀᴀᴡᴀʟ Pʀᴏᴄᴄᴇᴅᴇᴅ!!!!</code>\n<code>User --> </code><b><a href="tg://user?id='+ctx.from.id+'">'+ctx.from.first_name+'</a></b>\n<code>Amount --> '+msg+' '+bot_cur+'</code>\n<code>Wallet --> </code><code>'+wallet+'</code>\n<code>Payment Status --> </code><b><a href="https://polygonscan.com/tx/'+res+'">Successfully Paid✓</a></b>',{parse_mode: 'HTML', disable_web_page_preview: 'true' , reply_markup: { inline_keyboard:[[{ text: "Join "+bot_cur+" Airdrop ↗️", url: "https://t.me/"+ctx.botInfo.username+"?start="+ctx.from.id+"" }]]} })
                  console.log(res)                  
               }
           })
       }
...
function mustJoin(ctx){
  let msg = '<b>🕊 Join Telegram Communities</b>\n-------------------------------------------<b>'
  for(var ind in channels){
  var cha = channels[ind]
  msg+='\n--> '+cha
  }
  msg+='</b>\n-------------------------------------------\n<b>--> Follow Us On</b> <a href="https://twitter.com/airdropnft_twet">Twitter</a>\n<b>--> Subscribe Us On</b> <a href="https://www.youtube.com/@airdropsandnft"> YouTube</a>\n-------------------------------------------\n<b>--> Payout:- '+data.payment_channel+'</b>\n-------------------------------------------\n!?!<b>Before Using This Bot!?!</b>'
  ctx.replyWithHTML(msg, {reply_markup:{
   keyboard: [['⏺ Continue']],disable_web_page_preview : true,
